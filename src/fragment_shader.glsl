...
vec2 sdRobot(vec3 p0, float b) {
    ...
    p.zx *= rot(1.6 * k2);
    ph = p;
    min2(d2, vec2(V(p, .9, .6, .5, 0.), 4)); // tete
    p.x = abs(p.x);
...
void sdBook(inout vec2 r, vec3 p, float w, float h, float c) {
    ...
    p.z -= kr * .04 * cos(p.x / w) - .019;  // bombé
    min2(r, vec2(W(p, 0, 0, .015, w, h * .95 + .002 * cos(6e2 * p.x), h * .65, .0), 15)); // arriere
...
vec2 sdMap(vec3 p0) {
    ...
    b = pow(8., -9. * b) * cos(b * 63.);

    pl.y = mod(pl.y + 10., 17.5) - 10.;

    vec2 h,
        res0 = vec2(max(-hall,
            W(pl, 0, .1, 0, 999, .1, 999, 0.)), 3), // plancher
        res = sdRobot(p, b);
    ...
    p.z = abs(p.z);
    min2(res0, vec2(W(p, 0, 0, 2, .6, 0, 0, .05), 1.5));
    ...
    p.xz = abs(p.xz);
    p.xy *= rot(.1);
    min2(res0, vec2(W(p, 1.6, .6, .6, 0, .7, 0, .1), 8)); // pied tabouret
    ...
    if (kr < .5 || iTime < 85.) {
        res0 = vec2(sh == 0 ? p.x - c > .5 ? W(p, c + 11.5, .75, 0, 10.48, .8, .7, 0.) : W(p, c - 10.5, .8, 0, 10.48, .8, .7, 0.) : 999., 0);
        p.x -= c;
        for (x = .02; x <= .97;) {
            h = vec2(1. - .87 * kr, .4) + kr * vec2(.2, .3) * hash22(vec2(x, c + iy));
            h.x = min(h.x, .98 - x);
            x += h.x;
            if (h.x > .12 + .5 * kr * b)
...
vec3 render(vec3 ro, vec3 rd) {
    ...
    vec3 col, ref, lig, nor = vec3(0, max(1e-4 * t, .03 - kr * .02), 0);
    ...
    col =
        m < 3. ? fract(m + .99) * txWood2(ro, int(m)) :
        m == 3. ? .4 + .6 * vec3(txWood(ro).x) :
        m == 4. || m == 5. ? mix(vec3(.01, .3, 1), vec3(.6, .3, .1), smoothstep(.6, .7, tex3D(ph * 2., nor))) :
        m == 7. ? vec3(txInvader(ph.yz - vec2(.46, .05))) :
        col;
    ...
    lig += .5 * vec3(.05, .25, .5) * max(0., dot(rd, reflect(vec3(.97, 0, .24), nor)));
...
void mainImage(out vec4 cl, vec2 glFragCoord) {
    ...
    vec2 R =
        // vec2(1280,720), q=gl_FragCoord.xy,
        iResolution.xy, q = glFragCoord.xy,
        p = (q + q - R) / R.y; q /= R;
