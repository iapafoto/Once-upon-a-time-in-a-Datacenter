...
static WININFO wininfo = { 0,0,0,0,0, {'s','e','b',0} };
...
#ifdef SAVE_FRAME
    char buff[256];
    int* buffer = new int[XRES * YRES * 4];
    short  TGAhead[] = { 0, 2, 0, 0, 0, 0, XRES, YRES, 32 };
#endif
    while (!done)
    {
        long t = timeGetTime() - to;
#ifdef SAVE_FRAME
        t = (1000l*(long)iFrame) / 30l;
#endif
        ...
        SwapBuffers(info->hDC);
         ...
         glReadPixels(0, 0, XRES, YRES, GL_BGRA_EXT, GL_UNSIGNED_BYTE, buffer);
        // Then you can write the buffer into a.tga file :
         snprintf(buff, sizeof(buff), "%s%04d%s", "C:\\Users\\sebastien.durand\\Desktop\\anim\\tga_", iFrame, ".tga");
         FILE* out = fopen(buff, "w");
         fwrite(&TGAhead, sizeof(TGAhead), 1, out);
         fwrite(buffer, 4 * XRES*YRES, 1, out);
         fclose(out);
       //  out->close();
       //  free(buffer);
         delete[] buffer;
         buffer = new int[XRES * YRES * 4];
         fclose(out);
#endif
       
         Sleep(1);// 000); // give other processes some chance to do something
        iFrame++;
    }
