...
const char *input =
 ...
 "{"
   ...
   "if(i>3.1&&f==0)"
     "return vec2(i-3.,10);"
   "vec3 l=r,n=r,p;"
   "i=s(l,vec3(.3,.6,.2),.1+.1*l.g);"
   "vec2 t=vec2(999);"
   "d(t,l,vec3(0,.6,0),vec3(0,1.1,-.4),vec3(0,1.6,.2));"
   "l.r=abs(l.r);"
   "l.br*=x(.2);"
   "i=min(i,s(l-vec3(.4,-1.8,1.3),vec3(.2,.12,.4),0.));"
   ...
   "return t;"
 "}"
 "void n(inout vec2 v,vec3 r,float g,float b,float l)"
 "{"
   "r-=vec3(-g*.5,b+.07,0);"
   "g=g*.5-.015;"
   "r.r=abs(r.r);"
   ...
   "s(v,vec2(min(i,s(r-vec3(0,0,b*.7+.01-.02*(.3+cos(2.*max(.9,cos(14.*r.g/b))))),vec3(g-.015,b,0),.015)),l*l*l*l));"
 "}"
 "vec2 i(vec3 r)"
 "{"
   "vec3 l=r,i=r-vec3(-20,19.65,3);"
   "i.rb*=x(.4);"
   "float g,b=s(r-vec3(5,31,5),vec3(15,30,5),0.),c=max(0.,v-80.),p=length(i-vec3(0,2,0))-10.*smoothstep(0.,18.,c)-5.1*max(0.,c-19.),t=clamp((80.-v)/15.,0.,1.);"
   "t=pow(9.,-15.*t)*cos(t*30.);"
   "l.g=mod(l.g+10.,17.5)-10.;"
   "vec2 a,C=vec2(max(-b,s(l-vec3(0,.1,0),vec3(999,.1,999),0.)),3),o=d(i,t);"
   "m=smoothstep(-1.,1.,p);"
   "i=r-vec3(6.7,0,5);"
   "i.rg*=x(.785);"
   "i.b=abs(i.b);"
   ...
   "i.gr*=x(.785);"
   "s(C,vec2(min(s(i,vec3(.45,.5,3),.05),max(-b,s(l-vec3(3.*round(l.r/3.),-.5,0),vec3(.17,.45,999),0.))),2.3));"
   "i=r-vec3(82,0,-2.6);"
   "i.bg*=x(-.2);"
   "i.r=abs(i.r);"
   ...
   "s(o,vec2(max(-b,s(i,vec3(1e3,.07,.7),0.)),.99));"
   "i.g=mod(r.g-1.,2.5);"
   "g=floor(i.r);"
   "if(m<.5||v<85.)"
     "{"
       "C=vec2(f==0?"
         "i.r-g>.5?"
           "s(i-vec3(g+11.5,.75,0),vec3(10.48,.8,.7),0.):"
           "s(i-vec3(g-10.5,.8,0),vec3(10.48,.8,.7),0.):"
         "999.,0);"
       "i.r-=g;"
       "for(c=.02;c<=.97;)"
         "{"
           ...
           "c+=a.r;"
           "if(a.r>.12+.5*m*t)"
             "n(C,i-vec3(c,0,0),a.r,a.g,2.+s(a+g).r);"
         "}"
 ...
 "{"
   ...
   "if(a<0.)"
     "return l*=0.,vec3(.7,.9,1)*(a<-1.2&&a>-2.?"
       ...
       "1.);"
   "r+=x*l;"
   "vec3 F=vec3(0,max(1e-4*x,.03-m*.02),0),H=vec3(i(r-F.grr).r,i(r-F.rgr).r,i(r-F.rrg)),e=vec3(i(r+F.grr).r,i(r+F.rgr).r,i(r+F.rrg)),E=e-H+.001;"
   ...
   "for(int D=0;++D<28&&a>.001&&o<2.5;)"
     "a=i(r+H*o).r,c=min(c,2.*a/o),o+=clamp(a,.02,.1);"
   "o=1.05;"
   "a=1.;"
 ...
 "void main()"
 "{"
   "vec2 m=vec2(1280,720),i=gl_FragCoord.rg,l=(i+i-m)/m.g;"
   "i/=m;"
   "int[18] g=int[18](0,10,10,20,20,35,20,40,20,47,30,55,55,65,92,99,99,130);"
   ...
   "for(int e=0;e<9;e++)"
     "a=smoothstep(float(g[e*2]),float(g[e*2+1]),v),o=mix(o,vec3(c[e*3+3],c[e*3+4],c[e*3+5]),a),p=mix(p,vec3(b[e*3+3],b[e*3+4],b[e*3+5]),a);"
   "a=.5*cos(.2*v);"
   "t=normalize(p-o);"
   "p=normalize(cross(t,vec3(sin(a),cos(a),0)));"
   "p=mat3(p,normalize(cross(p,t)),t)*normalize(vec3(l+(v>99.?"
     ".01*s(l-l+v).r:"
     "0.),2));"
   "a=50.;"
   "C=d(o,p,a);"
   "if(p.r!=0.)"
     "C+=a*.2*d(o,p,f);"
   "a=smoothstep(125.,132.,v);"
   "r=vec4(pow(clamp(mix(C,vec3(.1,.5,1),smoothstep(1.,1.3,n(2.5*l)+a))*(1.-a),0.,1.),vec3(.6))*4.*sqrt(i.r*i.g*(1.-i.r)*(1.-i.g)),1);"
   "r=v<135.?"
     "r:"
     "vec4(1&int[](0,~0,124,3236969472,4160487169,~0,0,2199966623,~0,2031632,0)[min(int(length(l*20.)),10)]>>int(v+5.*atan(l.g,l.r)));"
 "}";
