...
float smoothstep(float x) {  // smoothstep(0,1,x)
    x = clamp(x, p0d00, p1d00);
    return x * x * (3.f - x-x);
}
...
//4076 (apres pusieurs rebuild !??
void mzk_init(short* buffer) {  //4115    15 octet le smoothstep
    for (int i = 0; i < MZK_NUMSAMPLES; i++) {
        ...
        buffer[i] = f2i(y* 4915.f/*p0d15 * 32767.f*/);
    }
}
